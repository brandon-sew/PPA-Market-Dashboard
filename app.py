...
if len(d_range) == 2:
    ...
    if not full_price_df.empty:
        ...
        full_price_resampled = full_price_df.groupby('Zone', group_keys=False).apply(lambda x: x.set_index('Time').resample(freq).mean(numeric_only=True).ffill()).reset_index()
        ...
        if not forecast_df_raw.empty:
            forecast_df = forecast_df_raw.groupby('Zone', group_keys=False).apply(lambda x: x.set_index('Time').resample(freq).mean(numeric_only=True).ffill()).reset_index()
